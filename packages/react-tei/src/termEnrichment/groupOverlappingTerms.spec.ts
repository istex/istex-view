...
describe("groupOverlappingTerms", () => {
	...
	it("should return a single group with one term when given a single term", () => {
		const terms: TermWithPosition[] = [
			{ term: { targetText: "term", groups: ["group1"] }, start: 0, end: 5 },
		];
		const result = groupOverlappingTerms(terms);
		expect(result).toEqual([
			[
				{
					term: { targetText: "term", groups: ["group1"] },
					start: 0,
					end: 5,
				},
			],
	...
	it("should return separate groups for non-overlapping terms", () => {
		const terms: TermWithPosition[] = [
			{ term: { targetText: "hello", groups: ["group1"] }, start: 0, end: 5 },
			{ term: { targetText: "world", groups: ["group1"] }, start: 10, end: 15 },
		];
		const result = groupOverlappingTerms(terms);
		expect(result).toEqual([
			[
			...
			[
				{
					term: { targetText: "world", groups: ["group1"] },
					start: 10,
					end: 15,
				},
			],
	...
	it("should group overlapping terms together", () => {
		const terms: TermWithPosition[] = [
			...
			{ term: { targetText: "world", groups: ["group2"] }, start: 6, end: 11 },
		];
		const result = groupOverlappingTerms(terms);
		expect(result).toEqual([
			[
	...
	it("should group multiple overlapping terms in the same group", () => {
		const terms: TermWithPosition[] = [
			{
				term: { targetText: "United States of America", groups: ["group1"] },
				start: 0,
				end: 24,
			},
			{
				term: { targetText: "States of America", groups: ["group2"] },
				start: 7,
				end: 24,
			},
			{
				term: { targetText: "America", groups: ["group3"] },
				start: 17,
				end: 24,
			},
		];
	...
	it("should handle adjacent but non-overlapping terms as separate groups", () => {
		const terms: TermWithPosition[] = [
			{ term: { targetText: "hello", groups: ["group1"] }, start: 0, end: 5 },
			{ term: { targetText: "world", groups: ["group1"] }, start: 5, end: 10 },
		];
		const result = groupOverlappingTerms(terms);
		expect(result).toEqual([
			[
	...
	it("should handle a mix of overlapping and non-overlapping terms", () => {
		const terms: TermWithPosition[] = [
			...
			{ term: { targetText: "City", groups: ["group1"] }, start: 20, end: 24 },
		];
		const result = groupOverlappingTerms(terms);
		expect(result).toEqual([
			[
	...
	it("should handle a chain of overlapping terms extending the group", () => {
		const terms: TermWithPosition[] = [
			{ term: { targetText: "A", groups: ["g1"] }, start: 0, end: 5 },
			{ term: { targetText: "B", groups: ["g2"] }, start: 3, end: 8 },
			{ term: { targetText: "C", groups: ["g3"] }, start: 6, end: 11 },
		];
		const result = groupOverlappingTerms(terms);
	...
	it("should handle multiple separate groups of overlapping terms", () => {
		const terms: TermWithPosition[] = [
			...
			{ term: { targetText: "dog owner", groups: ["g2"] }, start: 20, end: 29 },
		];
		const result = groupOverlappingTerms(terms);
		expect(result).toEqual([
			[
